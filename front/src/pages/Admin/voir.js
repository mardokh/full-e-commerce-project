...
const EditProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setProduct({id: null, name: "", price: "", details: "", image: "", images: []})
                    ...
                    <div className="edit_product_image_container" ref={imagesDisplayContainerRef}>
                        {product.images.map((image, index) => (
                            <div className="edit_product_images_container" key={index}>
                                ...
                                <div className="edit_product_images" style={{backgroundImage: `url('${!imageFlag.current ? `http://localhost:8989/uploads/${image.images}` : URL.createObjectURL(image)}')`}}></div>
                            </div>
                        ))}
                    </div>
                </div>
